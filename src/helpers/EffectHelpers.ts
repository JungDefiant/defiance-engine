...
import { AbilityData, AbilityDescriptor, AbilityTrigger, ActorData, EffectVar } from "src/components/ActorData";
import { Themes } from "src/gui/Themes";
import GameState from "src/states/GameState";
import RenderQueueSystem, { RenderQueueEntry, RenderQueueType } from "src/systems/RenderQueueSystem";
...
export function processAbilityEffects(
    ...
    abilityData.effectData.forEach((eff) => {
        switch (eff.id) {
            case "damage":
                effText = applyDamageEffect(sourceData, targetData, abilityData.descriptors, {
                    ...eff.variables,
                    ...context,
                });

                addFloatingTextRQE(targetData.entityId, effText, Themes.neutral2);
                gs.combatHud.addCombatLogEntry(`${sourceData.name} (${abilityData.name})`, `Inflicts ${effText} Damage to ${targetData.name}!`);
                break;
            case "healing":
                effText = applyHealEffect(sourceData, targetData, abilityData.descriptors, {
                    ...eff.variables,
                    ...context,
                });
                addFloatingTextRQE(targetData.entityId, effText, Themes.success);
